...
class SyncQvaPayUserClient:
    ...
    * access_token: Bearer token obtained from login.
    ...
    def topup(self, pay_method: str, amount: float) -> dict[str, Any]:
    ...
    def pay(self, uuid: Union[str, UUID], pin: str) -> dict[str, Any]:
    ...
    def get_p2p_coins_list(self) -> List[Coin]:
        """Get P2P enabled currencies."""
        response = self.http_client.get("p2p/get_coins_list")
        validate_response(response)
