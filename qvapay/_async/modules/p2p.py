...
class P2PModule:
    ...
    async def completed_pairs_average(self, coin: str) -> Any:
    ...
    async def total_public_open_ops(self) -> Any:
        ...
        response = await self._http.get("p2p/get_total_operations")
        validate_response(response)
        return response.json()

    async def get_offers(self, **filters: Any) -> List[P2POffer]:
        """Get all P2P offers."""
        response = await self._http.get(
            "p2p",
    ...
    async def get_my_offers(self, **filters: Any) -> List[P2POffer]:
        """Get my P2P offers."""
        response = await self._http.get(
            "p2p",
    ...
    async def get_offer_public(self, uuid: str) -> P2POffer:
        ...
        response = await self._http.get(f"p2p/{uuid}/pub")
        validate_response(response)
        return P2POffer.from_json(response.json())

    async def create_offer(self, **kwargs: Any) -> P2POffer:
        """Create a new P2P offer."""
        response = await self._http.post("p2p/create", json=kwargs)
        validate_response(response)
        return P2POffer.from_json(response.json())
    ...
    async def mark_paid(self, uuid: str) -> Any:
        ...
        response = await self._http.post(f"p2p/{uuid}/paid")
        validate_response(response)
        return response.json()

    async def confirm_received(self, uuid: str) -> Any:
        """Confirm received for a P2P offer."""
        response = await self._http.post(f"p2p/{uuid}/received")
        validate_response(response)
        return response.json()
